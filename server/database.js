...
export async function insertProduct(business_id, category_name, product_name, product_description, quantity, reorder_level, reorder_quantity, price, supplier_ids) {
    const sqlProduct = `
    ...
    try {
        ...
        // Assuming supplier_ids is an array of supplier IDs provided for the product
...
export async function getSupplierByBusiness(supplier_id, business_id) {
    const sql = `
        SELECT * FROM SUPPLIERS 
...
export async function insertOrder(business_id, product_id, quantity, price, order_date) {
    const sqlOrder = `
        INSERT INTO ORDERS (BUSINESS_ID, PRODUCT_ID, QUANTITY, PRICE, ORDER_DATE)
        VALUES (?, ?, ?, ?, ?);
    `;
    try {
        const checkBusiness = await checkBusinessExists(business_id);
        ...
        const [orderResult] = await pool.query(sqlOrder, [business_id, product_id, quantity, price, order_date]);
...
export async function updateOrder(order_id, business_id, updates) {
