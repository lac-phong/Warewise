...
app.post('/register', async (req, res) => {
    ...
    const user = await insertBusiness( username, password, business_name, address )
...
app.post("/orders", async (req, res) => {
    const { business_id, product_id, quantity, price } = req.body;
    const orders = await insertOrderWithDetails(business_id, product_id, quantity, price);
    res.status(201).send(orders);
});
